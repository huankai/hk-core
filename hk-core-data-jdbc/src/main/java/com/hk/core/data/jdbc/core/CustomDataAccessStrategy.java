...
//            + "JDBC driver returns it.";
     ...
     */
    public CustomDataAccessStrategy(SqlGeneratorSource sqlGeneratorSource, RelationalMappingContext context,
    ...
    public <T> Iterable<T> findAllById(Iterable<?> ids, Class<T> domainType) {
        long size = CollectionUtils.size(ids);
        if (size == 0) {
            return Collections.emptyList();
        }
        if (size == 1) {
            return ArrayUtils.asArrayList(findById(ids.iterator().next(), domainType));
        } else {
